# Diseño del Curso 4 — 🧭 Los 8 Ámbitos de Gestión

**Línea:** Desarrollo Institucional · **Nivel:** 1 (Fundamentación) · **Posición:** Curso 4 de 6 del Nivel 1.

> Recorrido panorámico de los 8 ámbitos de gestión que la PNDI 2017 identifica como el contenido operativo del Desarrollo Institucional. **Una micro-historia por ámbito** (no 8 definiciones). Al final, el adulto sabe ubicar cualquier decisión cotidiana del grupo en el ámbito que le corresponde.

---

## 1. Ficha del curso

| Campo | Valor |
|---|---|
| `courseId` | `los-8-ambitos-de-gestion` |
| Título | Los 8 Ámbitos de Gestión |
| Subtítulo | Curso 4 de la Línea Desarrollo Institucional · Nivel 1 — Fundamentación |
| Icono | 🧭 |
| Duración | ~35 min |
| Lecciones de contenido | 6 + intro + certificado |
| Audiencia primaria | La misma de la Línea. En especial: miembros del consejo de grupo y regional, comisionados de cualquier especialidad, jefes de grupo. |
| Pre-requisitos | Curso 2 (recomendado, para tener el marco PNDI completo). |
| Logro final | "Ámbitos a la vista" |

---

## 2. Objetivos del curso

Al completar este curso, el adulto:

1. **Identifica** los 8 ámbitos de gestión definidos por la PNDI 2017 y los puede recitar con sus emojis-ancla.
2. **Reconoce** una micro-historia ilustrativa por cada ámbito (caso real con detalles cambiados).
3. **Ubica** cualquier decisión cotidiana del grupo en el ámbito al que pertenece.
4. **Distingue** los 8 ámbitos PNDI 2017 (visión conceptual) de las 5 subcategorías operativas DNDI vigentes (organización actual) y entiende por qué ambas conviven.
5. **Conecta** los 8 ámbitos con el catálogo de buenas prácticas (5 atributos de la PNDI 2017, Cap. 10) que verá en el Curso 5.
6. **Sale** habiendo identificado los 2-3 ámbitos donde más oportunidad de aporte tiene en su grupo o región.

---

## 3. Hook pedagógico

> **"Cuando puedes ponerle nombre al ámbito al que pertenece una decisión, dejas de improvisar y empiezas a gestionar."**

Se enuncia en la Lección 1, se referencia en cada lección de ámbitos (3, 4, 5) y se cierra en la Lección 6.

---

## 4. Estructura de lecciones

### 4.1 Mapa general

| # | Lección | Duración | Idea central | Logro al completar |
|---|---|---|---|---|
| 1 | 👋 Bienvenida | 3 min | "Ponerle nombre a los ámbitos te da poder de gestión." | Empecé el recorrido |
| 2 | 🗺️ Mapa de los 8 ámbitos (vista panorámica) | 5 min | Los 8 ámbitos cubren TODO el trabajo institucional del grupo, sin huecos ni dobles. | Veo el mapa completo |
| 3 | 🏛️ Ámbitos 1-3 — Gobernanza, Administración, Gestión de Recursos Económicos | 8 min | Estos tres ámbitos sostienen la **operación interna** del grupo. | Domino los ámbitos internos |
| 4 | 🌐 Ámbitos 4-6 — Comunicaciones, Relaciones Internacionales, Crecimiento | 8 min | Estos tres ámbitos conectan al grupo con el **exterior y el futuro**. | Domino los ámbitos hacia afuera |
| 5 | 🛡️ Ámbitos 7-8 — Gestión del Riesgo, Control y Reconocimiento | 6 min | Estos dos ámbitos **protegen** al grupo de lo que puede salir mal y le dan estímulo a lo que sale bien. | Domino los ámbitos protectores |
| 6 | 🎯 Tus ámbitos prioritarios + 8 ámbitos vs 5 subcategorías DNDI | 5 min | El adulto identifica 2-3 ámbitos donde puede aportar más. Y entiende la convivencia entre la visión conceptual (8 ámbitos PNDI) y la organización operativa actual (5 subcategorías DNDI). | Ámbitos a la vista |

**Total estimado: ~35 min.** Coincide con el plan maestro.

---

### 4.2 Lección 1 — 👋 Bienvenida (3 min, `isIntro: true`)

**Idea central:** "Ponerle nombre a los ámbitos te da poder de gestión."

**Secciones (en orden):**

1. **`info-box`** — Tiempo estimado (~35 min, el más largo del Nivel 1) y promesa: _"Al final vas a tener el mapa completo de los 8 ámbitos del Desarrollo Institucional, sabrás ubicar cualquier decisión de tu grupo en el ámbito correcto, y habrás identificado tus 2-3 ámbitos prioritarios para aportar."_
2. **`paragraph`** — Apertura con caso: _"En una reunión de consejo de grupo, un dirigente propone: 'Deberíamos mandar a alguien al congreso scout regional el próximo mes'. La conversación se va al costo (¿alcanza el presupuesto?), al permiso (¿quién lo aprueba?), al impacto (¿qué ganamos?). Lo que la conversación NO menciona es que esa propuesta toca al menos 4 ámbitos del DI: Gobernanza (decisión del consejo), Recursos Económicos (presupuesto), Comunicaciones (vamos a representar al grupo) y Crecimiento (oportunidad de aprendizaje). **Cuando ves los ámbitos, la conversación se ordena.**"_
3. **`heading` (nivel 3)** — _"Lo que vas a vivir en este curso"_
4. **`list`** — Las 5 ideas centrales de las Lecciones 2-6.
5. **`mission-box`** — _"Tip de uso: este es el curso más denso del Nivel 1 — son 8 ámbitos en 35 minutos. No intentes memorizarlos. Concéntrate en **reconocer cada ámbito por su historia**, no por su definición. Eso es lo que vas a usar el resto de tu vida scout."_

**Reflexión:** ninguna.

**Quiz:** ninguno.

**Logro al completar:** "Empecé el recorrido".

---

### 4.3 Lección 2 — 🗺️ Mapa de los 8 ámbitos (vista panorámica) (5 min)

**Idea central:** Los 8 ámbitos cubren TODO el trabajo institucional del grupo, sin huecos ni dobles. Conocerlos es como tener el plano de tu casa.

**Secciones (en orden):**

1. **`info-box`** — Idea central de la lección.
2. **`paragraph`** — _"La PNDI 2017 hizo un ejercicio de cartografía: dividió todo lo que se hace en un grupo, una región o la nación para sostener la institución, en 8 cajas grandes. Esas cajas son los **8 ámbitos de gestión**. Cubren la totalidad del trabajo institucional. Cuando una decisión no entra en ningún ámbito, casi seguro hay algún error de diseño — y cuando entra en varios a la vez (como en el ejemplo del congreso de la lección anterior), eso te dice que es una decisión institucionalmente compleja."_
3. **`heading` (nivel 3)** — _"🧭 Los 8 ámbitos de un vistazo"_
4. **`method-grid`** — 8 tarjetas, una por ámbito:
    - 🏛️ **1. Gobernanza** → _"Cómo se toman las decisiones, quién decide qué, cómo se documentan."_ (color morado)
    - 🗂️ **2. Administración** → _"Activos, RR.HH. del grupo, obligaciones legales, servicios generales. La administración interna."_ (color azul oscuro)
    - 💰 **3. Gestión de Recursos Económicos** → _"De dónde viene la plata, cómo se gestiona, qué proyectos se financian, márgenes y sostenibilidad."_ (color verde)
    - 📣 **4. Comunicaciones y Relaciones Interinstitucionales** → _"Cómo se comunica el grupo internamente, con su comunidad, con la prensa, con otras organizaciones."_ (color naranja)
    - 🌐 **5. Relaciones Internacionales** → _"Vínculo con OMMS, con OSN de otros países, con multilaterales (ONU, UNESCO, UNICEF)."_ (color azul claro)
    - 📈 **6. Crecimiento y Sistema de Información** → _"Cómo crece la membresía, cómo se registra y se hace seguimiento, las bases de datos del grupo."_ (color amarillo)
    - 🛡️ **7. Gestión del Riesgo** → _"Ambiente seguro, protección financiera, actividades seguras. Lo que evita que las cosas salgan mal."_ (color rojo)
    - 🏅 **8. Control y Reconocimiento** → _"Vigilancia, fiscalización, estímulos y reconocimientos. Lo que premia lo bueno y previene lo malo."_ (color dorado)
5. **`info-box`** — Sub-ámbitos: _"Cada uno de los 8 ámbitos tiene **sub-ámbitos** que lo desagregan. Por ejemplo, **Administración** incluye 7 sub-ámbitos: recursos financieros, activos fijos y variables, RRHH del grupo, pasivos y obligaciones, unidades de negocio, obligaciones legales con el Estado, servicios generales de planta física. En este curso ves el panorama; los Cursos 9-14 del Nivel 2 entran al detalle de cada uno."_
6. **`policy-quote`**:
    - `label`: "📋 Ver lista oficial de la PNDI"
    - `text`: _"Los ámbitos de gestión del Desarrollo Institucional son: 8.2.1 Gobernanza; 8.2.2 Administración; 8.2.3 Gestión de Recursos Económicos; 8.2.4 Comunicaciones y Relaciones Interinstitucionales; 8.2.5 Relaciones Internacionales; 8.2.6 Crecimiento y Sistema de Información; 8.2.7 Gestión del Riesgo; 8.2.8 Control y Reconocimiento."_
    - `source`: "Política Nacional de Desarrollo Institucional 2017, Capítulo 8, Sección 8.2"

**Reflexión:** _"Mira la lista de los 8 ámbitos. Sin pensar mucho: ¿cuáles dos son los que crees que tu grupo trabaja mejor? ¿Cuáles dos son los más descuidados? Tu intuición vale — vamos a contrastarla al final del curso."_

**Quiz (2 preguntas):**

> **P1.** Un consejo de grupo revisa su malla de trabajo anual y encuentra que solo tiene actividades para 5 de los 8 ámbitos de gestión. Un dirigente dice: "No hay problema, lo demás no nos aplica." ¿Qué le respondes según esta lección?
>
> a) _Tiene razón: cada grupo elige libremente cuántos ámbitos trabajar según su tamaño._
> b) _Los 8 ámbitos cubren TODO el trabajo institucional sin huecos ni dobles — que 3 no tengan actividades visibles no significa que no apliquen, sino que probablemente están descuidados o invisibilizados._ ✅
> c) _No hay problema, esos ámbitos los cubre directamente la región, no el grupo._

> **P2.** Una propuesta del consejo de grupo que toca a la vez Gobernanza, Gestión de Recursos Económicos y Comunicaciones es…
>
> a) _Una propuesta mal hecha que debería separarse en tres._
> b) _Una propuesta institucionalmente compleja que requiere análisis cruzado de varios ámbitos._ ✅
> c) _Una propuesta que solo el comisionado nacional puede aprobar._

**Logro:** "Veo el mapa completo".

---

### 4.4 Lección 3 — 🏛️ Ámbitos 1-3 — Gobernanza, Administración, Gestión de Recursos Económicos (8 min)

**Idea central:** Estos tres ámbitos sostienen la operación interna del grupo. Sin ellos, lo demás se cae.

**Secciones (en orden):**

1. **`info-box`** — Idea central de la lección.
2. **`paragraph`** — _"Empezamos por adentro. Estos tres ámbitos —cómo decidimos, cómo nos administramos, de dónde sacamos la plata— son la columna interna del grupo. En grupos que llevan años sosteniéndose, estos tres están aceitados. En grupos que se rompen, casi siempre falla al menos uno de los tres."_
3. **`heading` (nivel 3)** — _"🏛️ Ámbito 1 — Gobernanza"_
4. **`timeline`** — Micro-historia (1):
    - **🏛️ Caso Grupo 'Catatumbo':** _"El consejo de grupo decide grabar todas las sesiones (con consentimiento), generar acta resumida al cierre, firmarla todos antes de salir y publicarla la semana siguiente en la carpeta compartida. Tres años después, cuando llega un padre a reclamar por una decisión vieja, en 4 minutos sacan la grabación y el acta. La reclamación se atiende sin tensión: hay evidencia y trazabilidad. **Gobernanza sana.**"_
5. **`paragraph`** — _"¿Qué hizo posible eso? El grupo Catatumbo tenía resuelto su ámbito de **Gobernanza**: cómo se decide, quién decide, cómo se documenta. La PNDI 2017 (Cap. 8.2.1) lo describe en 5 elementos: **Gobierno** (los Consejos, elegidos, que conducen la institución), **Regulaciones** (las normas internas que se crean, revisan o eliminan), **Estructuras** (los órganos definidos en estatutos y reglamentos — Curso 3), **Funciones** (las tareas de cada órgano y de quienes lo integran) y **Planificación Estratégica** (los objetivos y plazos que el grupo se pone, mirando oportunidades, amenazas, fortalezas y debilidades). El acta y la grabación de Catatumbo son 'Funciones' y 'Regulaciones' hechas carne."_
6. **`heading` (nivel 3)** — _"🗂️ Ámbito 2 — Administración"_
7. **`paragraph`** — _"¿Qué hizo posible eso? El grupo Coraje tenía resuelta su **Administración**: sabe qué tiene, qué se dañó y quién respondió. La PNDI agrupa esto en 7 sub-ámbitos: **recursos financieros**, **activos fijos y variables** (sede, carpas, equipos), **recursos humanos** (los dirigentes, los voluntarios), **pasivos y obligaciones**, **unidades de negocio** (si el grupo opera una cantina, una tienda scout), **obligaciones legales con el Estado** (cámara de comercio, retenciones, impuestos), y **servicios generales de planta física** (luz, agua, mantenimiento de la sede). El inventario semestral de Coraje es 'activos fijos y variables' hecho rutina."_ (movido después del caso — patrón historia-antes-que-definición)
8. **`timeline`** — Micro-historia (2):
    - **🗂️ Caso Grupo 'Coraje':** _"El intendente hace inventario de carpas, equipos, libros y herramientas cada 6 meses, antes y después de la temporada de campamentos. Cuando un equipo se daña, queda registrado quién lo reportó y cuándo. Cuando llega un dirigente nuevo, no tiene que adivinar qué tienen — abre la planilla. **Administración sana.**"_
9. **`heading` (nivel 3)** — _"💰 Ámbito 3 — Gestión de Recursos Económicos"_
10. **`paragraph`** — _"¿Qué hizo posible ese margen? Sumapaz tenía resuelta su **Gestión de Recursos Económicos**: las **6 fuentes de fondos** clásicas del scoutismo (cuotas, eventos del grupo, servicios a terceros, aliados, donaciones, convocatorias), la planeación financiera, el manejo de proyectos. Muchos grupos sanos se ponen una meta propia de margen para tener colchón — no es un número que exija la PNDI, pero sí una buena práctica de planeación financiera. Aquí entra también el **ciclo de proyectos** (11 fases del Project Cycle de WOSM) cuando el grupo postula a fondos externos, como hizo Sumapaz con el fondo municipal."_ (movido después del caso — patrón historia-antes-que-definición)
11. **`timeline`** — Micro-historia (3):
    - **💰 Caso Grupo 'Sumapaz':** _"Llegan a fin de año con margen de 22 % sobre ingresos. ¿Por qué? Porque diversificaron: cuota mensual familiar, kermés trimestral con el barrio, servicio de catering en eventos comunitarios (una vez al mes), una empresa local que aporta como aliado anual, y postulación a un fondo municipal. Cuando un padre pierde el trabajo y no puede pagar la cuota, el grupo le da beca con cargo a esa reserva — sin que el resto del año se afecte. **Recursos Económicos sanos.**"_
12. **`info-box`** — Conexión: _"Estos tres ámbitos se ven en detalle en los Cursos 7 (Gobernanza Práctica), 9 (Administración del Grupo) y 10 (Finanzas Sanas) del Nivel 2. Acá viste el panorama; el detalle viene después."_

**Reflexión:** _"De los 3 ámbitos de esta lección, escribe en una frase qué tan saludable está cada uno en tu grupo — usando una escala simple: 🟢 sano, 🟡 con observaciones, 🔴 frágil. Y la razón en pocas palabras."_

**Quiz (2 preguntas):**

> **P1.** El intendente del grupo "Coraje", además de las carpas y equipos, empieza a llevar registro de a quién le pagan honorarios ocasionales y qué contratos de arriendo tiene vigentes con el salón comunal. Fuera de "activos", ¿en qué otros dos sub-ámbitos de Administración está trabajando?
>
> a) _Pasivos y obligaciones (los contratos vigentes) y obligaciones legales con el Estado (si esos honorarios implican retenciones)._ ✅
> b) _Gestión de Recursos Económicos, porque todo lo que involucra plata es ese ámbito._
> c) _Ninguno más — eso sigue siendo activos fijos y variables._

> **P2.** Si tu grupo opera la cuota mensual, una kermés anual y aspira a postular a un fondo municipal, está trabajando principalmente en el ámbito de…
>
> a) _Gobernanza._
> b) _Gestión de Recursos Económicos._ ✅
> c) _Comunicaciones._

**Logro:** "Domino los ámbitos internos".

---

### 4.5 Lección 4 — 🌐 Ámbitos 4-6 — Comunicaciones, Relaciones Internacionales, Crecimiento (8 min)

**Idea central:** Estos tres ámbitos conectan al grupo con el exterior y con el futuro. Sin ellos, el grupo queda aislado y se agota.

**Secciones (en orden):**

1. **`info-box`** — Idea central de la lección.
2. **`paragraph`** — _"Si los tres ámbitos anteriores son la columna interna, estos tres son los brazos que llegan afuera y los ojos que miran al horizonte. Un grupo solo con columna interna se mantiene, pero no crece ni se renueva. Estos tres ámbitos son los que evitan que el grupo se convierta en una isla."_
3. **`heading` (nivel 3)** — _"📣 Ámbito 4 — Comunicaciones y Relaciones Interinstitucionales"_
4. **`paragraph`** — _"¿Qué hizo posible esa respuesta en 30 minutos? Iguaque tenía resueltas sus **Comunicaciones y Relaciones Interinstitucionales**: los 4 sub-ámbitos son **internas** (entre dirigentes, entre el consejo y las familias), **externas** (con la comunidad del barrio, redes sociales, prensa), **relaciones públicas** (eventos, alianzas con instituciones locales), e **interinstitucionales** (vínculo con la región, la nación, otras organizaciones). Incluye la **imagen institucional**, el manejo de **marca** y la **narrativa pública** del grupo. El comunicado de Iguaque fue 'comunicación interna y externa' funcionando a la vez."_ (movido después del caso — patrón historia-antes-que-definición)
5. **`timeline`** — Micro-historia (4):
    - **📣 Caso Grupo 'Iguaque':** _"Cuando hubo un accidente menor en un campamento, el jefe de grupo escribió un comunicado a las familias en 30 minutos: qué pasó, qué medidas se tomaron, qué van a hacer para que no se repita. Esa misma noche lo publicaron en el grupo de WhatsApp y por correo. Tres familias agradecieron la transparencia. Una preguntó cómo apoyar. **Cero familias se retiraron**. La diferencia entre rumor y comunicación oportuna es enorme. **Comunicaciones sanas.**"_
6. **`heading` (nivel 3)** — _"🌐 Ámbito 5 — Relaciones Internacionales"_
7. **`paragraph`** — _"¿Qué hizo posible esa conexión con Filipinas? Bachué activó su ámbito de **Relaciones Internacionales**: el vínculo del nivel correspondiente con la **OMMS** (Organización Mundial del Movimiento Scout), con la **Región Scout Interamericana**, con **OSN de otros países**, y con organismos multilaterales (ONU, UNESCO, UNICEF). A nivel de grupo, este ámbito generalmente se experimenta como **participación en proyectos mundiales** (Scouts for SDGs, Tide Turners, Earth Tribe, etc.) más que como diplomacia formal — justo como hizo Bachué con 'Champions for Nature'."_ (movido después del caso — patrón historia-antes-que-definición)
8. **`timeline`** — Micro-historia (5):
    - **🌐 Caso Grupo 'Bachué':** _"Postularon su proyecto de huerta comunitaria al programa global 'Champions for Nature' de WOSM. Lo presentaron en inglés con apoyo de la jefa de tropa que es traductora. Los aceptaron, recibieron un kit de implementación, conectaron con un grupo en Filipinas que hace algo parecido. El proyecto creció. Las chicas del clan publicaron un video del intercambio. **El scoutismo es global, pero solo si te conectas. Relaciones Internacionales operando.**"_
9. **`heading` (nivel 3)** — _"📈 Ámbito 6 — Crecimiento y Sistema de Información"_
10. **`paragraph`** — _"¿Qué hizo posible pasar de 28 a 65 chicos en 3 años? Pisamadores trabajó su **Crecimiento y Sistema de Información**: cómo crece la membresía (cuántos chicos y adultos entran y salen cada año), cómo se registra y se hace seguimiento (la base de datos del grupo, el sistema de identificación scout, los carnés), las estadísticas básicas, el apoyo a la membresía existente. La plataforma **SiScout** y el **Modelo de Servicios OMMS** entran acá. El **Plan Regional Interamericano 2022-2025** espera que al menos 17 países de la región reporten un crecimiento de membresía de al menos 2 % en todo el período (no por año) — no es una meta anual para tu grupo, pero sí una señal de qué tan ambicioso debería ser tu plan de crecimiento, como el de Pisamadores."_ (movido después del caso — patrón historia-antes-que-definición)
11. **`timeline`** — Micro-historia (6):
    - **📈 Caso Grupo 'Pisamadores':** _"Hace 4 años, el grupo tenía 28 chicos y 6 dirigentes. La comisionada regional de crecimiento les propuso un plan trienal: campaña de barrio una vez al año, jornada de puertas abiertas, mejorar el seguimiento de los que se inscriben pero no vuelven. Llevaron registro mes a mes en una hoja simple. Tres años después tienen 65 chicos y 11 dirigentes. Crecieron sostenidamente. **Crecimiento medido y planeado.**"_
12. **`info-box`** — Conexión: _"Estos tres ámbitos se ven en detalle en los Cursos 12 (Comunicaciones y Relaciones Interinstitucionales) y 13 (Crecimiento y Sistema de Información) del Nivel 2. Y en el Curso 24 del Nivel 4 (Articulación con Plan Trienal Mundial y Plan Regional) verás cómo se traduce este nivel a estrategia."_

**Reflexión:** _"De los 3 ámbitos de esta lección, escribe en una frase qué tan saludable está cada uno en tu grupo (🟢 / 🟡 / 🔴) y la razón."_

**Quiz (2 preguntas):**

> **P1.** Que el jefe de grupo escriba a las familias 30 minutos después de un accidente menor en campamento, con qué pasó, qué se hizo y cómo lo van a prevenir, es un ejemplo de…
>
> a) _Gobernanza._
> b) _Comunicaciones (sub-ámbito Comunicación Externa)._ ✅
> c) _Gestión del Riesgo (sub-ámbito ambiente seguro)._

> **P2.** Un comisionado regional le dice a un grupo pequeño: "Este año tienen que crecer mínimo 2%, es la meta del Plan Regional Interamericano." ¿Qué le corriges?
>
> a) _Nada, tiene razón: todo grupo debe crecer 2% cada año._
> b) _La meta es regional y de todo el período 2022-2025: que al menos 17 países crezcan 2% en total — no es una exigencia anual para cada grupo._ ✅
> c) _Tiene razón, pero la meta es 2% mensual, no anual._

**Logro:** "Domino los ámbitos hacia afuera".

---

### 4.6 Lección 5 — 🛡️ Ámbitos 7-8 — Gestión del Riesgo, Control y Reconocimiento (6 min)

**Idea central:** Estos dos ámbitos protegen al grupo: uno evita que las cosas salgan mal; el otro premia lo bueno y vigila lo malo.

**Secciones (en orden):**

1. **`info-box`** — Idea central de la lección.
2. **`paragraph`** — _"Ya viste cómo el grupo decide, se administra, se financia, se comunica, se conecta y crece. Faltan dos ámbitos críticos: el escudo (Gestión del Riesgo) y el espejo + premio (Control y Reconocimiento)."_
3. **`heading` (nivel 3)** — _"🛡️ Ámbito 7 — Gestión del Riesgo"_
4. **`paragraph`** — _"¿Qué hizo posible resolver ese esguince en 8 minutos? Llanos tenía resuelta su **Gestión del Riesgo**: las 3 áreas transversales de riesgo en el scoutismo según la doctrina vigente son **ambiente seguro** (A Salvo del Peligro, prevención de abuso), **protección financiera y patrimonial** (cuidar la plata y los bienes del grupo), y **actividades seguras** (campamentos, salidas, transporte, primeros auxilios). El **ciclo de gestión del riesgo** tiene 4 pasos: identificación, análisis, tratamiento y monitoreo. El protocolo de 15 minutos de Llanos es 'actividades seguras' hecho rutina."_ (movido después del caso — patrón historia-antes-que-definición)
5. **`timeline`** — Micro-historia (7):
    - **🛡️ Caso Grupo 'Llanos':** _"Antes de cada salida hacen un protocolo de 15 minutos: lista de participantes (con padres firmando autorización), hoja de salud por participante, contactos de emergencia, ruta y horarios, protocolo de transporte (¿quién maneja, qué vehículo, hay seguro?), kit de primeros auxilios, un adulto certificado en primeros auxilios. Cuando hubo un esguince en el cerro, lo manejaron en 8 minutos. La región los citó como modelo. **Gestión del Riesgo operando.**"_
6. **`heading` (nivel 3)** — _"🏅 Ámbito 8 — Control y Reconocimiento"_
7. **`paragraph`** — _"¿Qué hizo posible esos reconocimientos bien argumentados? Tatamá activó su ámbito de **Control y Reconocimiento**: los órganos de control (que viste en el Curso 3) y todo el sistema de **estímulos y reconocimientos** del scoutismo. Esto incluye el **Manual de Estímulos Nacionales** y el **Manual de la Orden al Mérito Scout** (ambos del CSN 2022), que regulan los reconocimientos a dirigentes y voluntarios destacados. **Premia lo que funciona, vigila lo que se desvía** — como hizo Tatamá con sus 20 minutos anuales de reconocimientos."_ (movido después del caso — patrón historia-antes-que-definición)
8. **`timeline`** — Micro-historia (8):
    - **🏅 Caso Grupo 'Tatamá':** _"Cada año, en la asamblea de grupo, dedican 20 minutos a reconocimientos formales: el dirigente del año (elegido por las familias), el chico con mayor progresión personal por rama, el aporte voluntario del año (un papá o mamá que se destacó), un pin para los dirigentes que cumplen 5, 10, 15 y 20 años. Es un momento sentido. Los reconocimientos no son protocolarios — están bien argumentados. El año siguiente, varios más quieren ese pin. **Reconocimiento que motiva.**"_
9. **`info-box`** — Conexión: _"El ámbito de Gestión del Riesgo se desarrolla en el Curso 14 del Nivel 2. El de Control y Reconocimiento se cruza con el Curso 20 del Nivel 3 (Órganos de control y disciplina) y el Curso 22 del Nivel 4 (Buenas Prácticas Institucionales) y el Curso 23 (Código de Honor, Ética y Conflictos de Intereses)."_

**Reflexión:** _"De los 2 ámbitos de esta lección, escribe en una frase cómo está cada uno en tu grupo (🟢 / 🟡 / 🔴). Y específicamente para Gestión del Riesgo: ¿cuándo fue la última vez que tu grupo revisó su protocolo de salidas?"_

**Quiz (2 preguntas):**

> **P1.** El ámbito de Gestión del Riesgo tiene 3 áreas transversales. ¿Cuáles?
>
> a) _Ambiente seguro, protección financiera-patrimonial, actividades seguras._ ✅
> b) _Gobernanza, administración, finanzas._
> c) _Salud física, salud mental, salud social._

> **P2.** El Manual de Estímulos Nacionales y el Manual de la Orden al Mérito Scout son herramientas del ámbito de…
>
> a) _Crecimiento y Sistema de Información._
> b) _Comunicaciones y Relaciones Interinstitucionales._
> c) _Control y Reconocimiento (sub-ámbito reconocimientos y estímulos)._ ✅

**Logro:** "Domino los ámbitos protectores".

---

### 4.7 Lección 6 — 🎯 Tus ámbitos prioritarios + 8 ámbitos vs 5 subcategorías DNDI (5 min)

**Idea central:** Identificas los 2-3 ámbitos donde puedes aportar más. Y entiendes la diferencia entre los 8 ámbitos PNDI (visión conceptual) y las 5 subcategorías operativas DNDI vigentes.

**Secciones (en orden):**

1. **`info-box`** — Idea central de la lección.
2. **`heading` (nivel 3)** — _"🎯 Tus 2-3 ámbitos prioritarios"_
3. **`paragraph`** — _"Ya pasaste por los 8 ámbitos con una historia cada uno. Ahora viene la pregunta clave: **¿dónde puedes aportar tú?**"_
4. **`mission-box`** — _"**📌 Identifica tus 2-3 ámbitos prioritarios:**<br>**Ámbito 1 prioritario:** __________ (porque ___)<br>**Ámbito 2 prioritario:** __________ (porque ___)<br>**Ámbito 3 prioritario (opcional):** __________ (porque ___)<br><br>Para escoger, cruza tres criterios:<br>1. ¿Cuál ámbito es el más débil hoy en mi grupo o región?<br>2. ¿Cuál ámbito conecta con mi cargo o experiencia personal?<br>3. ¿En cuál puedo hacer un aporte concreto en los próximos 3-6 meses?"_
5. **`heading` (nivel 3)** — _"🤔 ¿Por qué los 8 ámbitos PNDI no coinciden con las 5 subcategorías DNDI?"_
6. **`paragraph`** — _"Si entraste a scout.org.co/biblioteca/dndi, vas a notar algo interesante: la **Dirección Nacional de Desarrollo Institucional** no organiza sus documentos en 8 ámbitos, sino en **5 subcategorías**: A Salvo del Peligro, Calidad, Crecimiento, Gestión del Riesgo y Proyectos. ¿Es una contradicción? No."_
7. **`method-grid`** — 2 tarjetas:
    - 📜 **8 ámbitos PNDI 2017** → _"Visión **conceptual** del Desarrollo Institucional. Cubre TODO lo institucional del Movimiento. Es marco doctrinal."_ (color morado)
    - 🛠️ **5 subcategorías DNDI hoy** → _"Organización **operativa actual** de la Dirección Nacional. Refleja las prioridades del cuatrienio. Es marco práctico de trabajo."_ (color verde)
8. **`paragraph`** — _"La explicación: **Gobernanza** y **Control** están reguladas por el CSN (no por la DNDI), por eso no aparecen como subcategorías de la dirección. **Comunicaciones**, **Relaciones Internacionales** y **Administración** dependen de la Jefatura Scout Nacional como áreas transversales, no como direcciones independientes. La DNDI, en su organización operativa actual, se concentra en 5 prioridades clave del cuatrienio. **Ambas vistas son válidas — la PNDI define el universo, la DNDI organiza las prioridades de hoy.**"_
9. **`heading` (nivel 3)** — _"🗺️ Lo que viene"_
10. **`paragraph`** — _"El **Curso 5** te entrega un **catálogo interactivo de buenas prácticas** de tu grupo, recorriendo los 8 ámbitos con el filtro de los 5 atributos de la PNDI 2017 (Cap. 10). Y el **Curso 6** convierte ese catálogo en un plan personal de aporte, partiendo de los ámbitos prioritarios que ya identificaste hoy."_
11. **`blockquote`** — _"Cuando puedes ponerle nombre al ámbito al que pertenece una decisión, dejas de improvisar y empiezas a gestionar. Ya tienes el lenguaje. Lo que sigue es la práctica."_

**Reflexión:** _"Escribe tus 2-3 ámbitos prioritarios con su 'porque'. Esto se va a usar como insumo en el Curso 6 cuando construyas tu plan personal de aporte al DI."_

**Quiz (2 preguntas):**

> **P1.** Los 8 ámbitos de la PNDI 2017 y las 5 subcategorías operativas de la DNDI hoy…
>
> a) _Son lo mismo con nombres distintos._
> b) _Conviven: la PNDI define el universo conceptual completo (8 ámbitos), la DNDI organiza sus 5 prioridades operativas del cuatrienio dentro de ese universo._ ✅
> c) _Se contradicen y la DNDI debería reorganizarse para coincidir con la PNDI._

> **P2.** Camila es tesorera y sabe que en su grupo la Gestión de Recursos Económicos va muy bien, pero Comunicaciones está totalmente descuidada — nadie contesta el WhatsApp de padres a tiempo. A ella le apasiona el diseño gráfico y tiene tiempo libre los sábados. Aplicando los tres criterios de esta lección, ¿qué ámbito debería priorizar?
>
> a) _Recursos Económicos, porque es donde ya tiene experiencia como tesorera._
> b) _Comunicaciones — es el ámbito más débil hoy, conecta con una habilidad personal suya (diseño) y tiene capacidad real de aportar (tiempo los sábados)._ ✅
> c) _Ninguno: debe esperar a que la región le asigne un ámbito formalmente._

**Logro al completar:** "Ámbitos a la vista" (logro final del curso, `unlockOnModule: -1`).

---

## 5. Logros (achievements)

| ID | Nombre | Emoji | Desbloqueo |
|---|---|---|---|
| `achievement-1` | Empecé el recorrido | 👋 | Al completar Lección 1 |
| `achievement-2` | Veo el mapa completo | 🗺️ | Al completar Lección 2 |
| `achievement-3` | Domino los ámbitos internos | 🏛️ | Al completar Lección 3 |
| `achievement-4` | Domino los ámbitos hacia afuera | 🌐 | Al completar Lección 4 |
| `achievement-5` | Domino los ámbitos protectores | 🛡️ | Al completar Lección 5 |
| `achievement-final` | Ámbitos a la vista | 🧭 | Al completar el curso (`unlockOnModule: -1`) |

---

## 6. Conexiones cross-course

### 6.1 Hacia atrás

- La **L3 del Curso 2** introdujo el fundamento **Servicio y Apoyo (Integralidad)** — los 8 ámbitos son su concreción operativa.
- La **L4 del Curso 1** ya nombró brevemente algunos ámbitos en los 6 ejemplos cotidianos (secretario = gobernanza, tesorero = administración, etc.); este curso los desarrolla todos.
- El **Curso 3** mostró los órganos por nivel; este curso muestra qué tipo de trabajo institucional hacen.

### 6.2 Hacia adelante

- **L6 (ámbitos prioritarios) → Curso 5:** los 2-3 ámbitos prioritarios se contrastan con el catálogo de buenas prácticas del grupo.
- **L6 (ámbitos prioritarios) → Curso 6:** son el insumo principal del plan personal de aporte.
- **Todos los ámbitos → Cursos 7-14 del Nivel 2:** cada ámbito tiene un curso dedicado de profundización en el Nivel 2.
- **L5 (Gestión del Riesgo) → Línea Transversal "Safe from Harm"** (cuando se construya).

### 6.3 Cross-línea

- **A Salvo del Peligro** (sub-ámbito de Gestión del Riesgo) tiene su propia política nacional y conecta con la **Línea Política de Adultos** (PNAM Doc 22-23) y la **Línea Programa de Jóvenes**.

---

## 7. Tipos de sección utilizados

| Tipo | Lecciones donde aparece |
|---|---|
| `info-box` | L1, L2, L3, L4, L5, L6 (todas) |
| `paragraph` | Todas |
| `heading` (nivel 3) | L2, L3, L4, L5, L6 |
| `list` | L1 |
| `method-grid` | L2 (1, con 8 tarjetas), L6 (1, con 2 tarjetas) |
| `timeline` | L3 (3 micro-historias), L4 (3 micro-historias), L5 (2 micro-historias) → **8 micro-historias en total, una por ámbito** |
| `mission-box` | L1, L6 |
| `policy-quote` | L2 |
| `blockquote` | L6 |
| `reflection` | L2, L3, L4, L5, L6 (excepto L1 intro) |
| `quiz` | L2, L3, L4, L5, L6 (excepto L1 intro) |

**No requiere ningún tipo de sección nuevo.**

---

## 8. Validación contra el marco metodológico

| Criterio | Cumplimiento |
|---|---|
| Curso entre 20 y 40 min | ✅ ~35 min |
| Lecciones de 3–8 min | ✅ rango 3–8 min |
| Cada lección termina independiente | ✅ |
| Lenguaje conversacional, tutea | ✅ |
| Citas oficiales plegables (`policy-quote`) | ✅ L2 (PNDI Cap. 8) |
| Reflexión personal por lección | ✅ excepto intro |
| Mini-quiz 2 preguntas | ✅ |
| 4–6 logros + 1 final | ✅ 5 + 1 |
| L1 marcada `isIntro: true` sin quiz | ✅ |
| Conexión cross-course | ✅ con Cursos 1, 2, 3, 5, 6, 7-14, 22, 23 y Línea PdJ |
| Hook por curso, repetible | ✅ "Cuando puedes ponerle nombre al ámbito, dejas de improvisar y empiezas a gestionar." |

---

## 9. Riesgo señalado y antídoto aplicado

| Riesgo (según Recomendaciones-Cowork) | Antídoto aplicado |
|---|---|
| "Es el más denso. Riesgo de 'manual de oficina'" | **8 micro-historias** (una por ámbito) con nombre ficticio de grupo, narradas como casos vivos. Las definiciones operativas vienen DESPUÉS de la historia, no antes. Las normas técnicas (números de KPI, sub-ámbitos completos) van en el cuerpo del párrafo o en `info-box` complementarios, nunca en lugar del relato. |

---

## 10. Próximos pasos

1. Revisar este diseño con el dueño del proyecto.
2. Claude Code traduce a JSON en `05-Generador-Cursos/borradores/los-8-ambitos-de-gestion.json`.
3. `node build-course.js los-8-ambitos-de-gestion`.
4. Generar preview PDF y revisar.
5. Actualizar `02-Plataforma-Web/cursos.json`.
6. Commit + push.

---

_Documento de diseño del Curso 4, versión inicial — 18 de mayo de 2026. Diseñado por Cowork siguiendo el patrón del Curso 1 y las pautas del documento `Recomendaciones-Cowork-Diseno-Cursos.md`. Incluye la distinción explícita entre los 8 ámbitos PNDI conceptuales, las 5 subcategorías operativas DNDI vigentes y las 6 áreas estratégicas del PPTX FLOR DE LIS II._
